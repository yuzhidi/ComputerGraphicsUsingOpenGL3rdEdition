// Sandy Hill, January 30, 2000 for ECE661
...
void Camera :: raytrace(Scene& scn, int blockSize)
{
	...
	for(int row = 0; row < nRows; row += blockSize)
		for(int col = 0; col < nCols; col += blockSize)
		{
			...
			clr.set(scn.shade(theRay)); // get color of this ray
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<< rayTrace >>>>>>>>>>>>>>>>>>>>>>
void Camera :: raytrace(Scene& scn, IntRect c)
{
	glMatrixMode(GL_MODELVIEW); 
	glLoadIdentity(); //set to identity
	glMatrixMode(GL_PROJECTION);
	...
	cout << "c = (" << c.left << ", " << c.bott << ", " << c.right << ", " << c.top << ")\n";
	for(int row = c.bott; row < c.top; row++)
		for(int col = c.left; col < c.right; col++)
		{
			//build the ray's direction
			theRay.row = row; 
			theRay.col = col; // for screen extents
			theRay.recurseLevel = 0;
			theRay.numInside = 0; // ray is outside all objects.
			x = -W + col * blockWidth;
			y = -H + row * blockHeight;
